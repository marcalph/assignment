...
def main(**kwargs):
    ...
    if kwargs["diagnose"]:
        plot_learning_curves(clf,make_scorer(roc_auc_score), X, y, filename="assets/ouput/LC_baseline.png", cv=sss, n_jobs=3)
        cv_confusion_matrix(clf, X, y, sss, filename="assets/output/CM_baseline.png", normalize=False)
        cv_confusion_matrix(clf, X, y, sss, filename="assets/output/CMprec_baseline.png", normalize=True)
    if kwargs["train"]:
